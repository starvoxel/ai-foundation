 ...
 * @param {import('../manifest.js').ManifestData} manifest
  ...
  for (const key of keys) {
    ...
    for (const serverName of entry.servers || []) {
      ...
      const f = checkFreshness(
        serverEntry && serverEntry.sourceHashes,
        readServerSnapshot(serverName, repoRoot),
      );
    ...
    for (const hookName of entry.hooks || []) {
      ...
      const f = checkFreshness(
        hookEntry && hookEntry.sourceHashes,
        readHookSnapshot(hookName, repoRoot),
      );
      if (f.hasFreshness && f.stale) dependencyStale = true;
    }
    ...
    console.log(
      `      ${integrity.total} files: ${integrity.current} current, ${integrity.stale} stale, ${integrity.missing} missing`,
    );
    ...
    if (dependencyStale) {
      console.log(
        '      One or more server/hook dependencies are stale (see Servers/Hooks below).',
      );
    }
 ...
 * for both.
 * @param {string} title
 * @param {Record<string, Record<string, any>>} sectionEntries - manifest.servers or manifest.hooks
 * @param {(name: string, repoRoot: string) => { sources: Record<string, string> }|null} readResourceSnapshot
 * @param {string} repoRoot
 * @returns {void}
 */
function reportSharedResourceSection(title, sectionEntries, readResourceSnapshot, repoRoot) {
  ...
  for (const key of keys) {
    ...
    const hasIssues =
      integrity.stale > 0 ||
      integrity.missing > 0 ||
      Boolean(freshness.hasFreshness && freshness.stale);
