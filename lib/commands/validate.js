...
function validateAgentSchemas(repoRoot) {
  ...
  for (const file of files) {
    ...
    if (!agent) { errors.push(`${file}: invalid YAML`); continue; }

    const name = file.replace(/\.yaml$/, '');
    if (!agent.name) errors.push(`${file}: missing 'name'`);
    else if (agent.name !== name) errors.push(`${file}: name '${agent.name}' doesn't match filename`);
    ...
    if (!Array.isArray(agent.approved_tools)) errors.push(`${file}: missing or invalid 'approved_tools'`);
...
function validateSkillSchemas(repoRoot) {
  ...
  for (const folder of folders) {
    ...
    if (!fm) { errors.push(`skills/${folder}/SKILL.md: missing or invalid frontmatter`); continue; }

    if (!fm.name) errors.push(`skills/${folder}/SKILL.md: missing 'name'`);
    else if (fm.name !== folder) errors.push(`skills/${folder}/SKILL.md: name '${fm.name}' doesn't match folder`);
    else if (!KEBAB_CASE.test(fm.name)) errors.push(`skills/${folder}/SKILL.md: name is not kebab-case`);
...
function validateSteeringSchemas(repoRoot) {
  ...
  for (const relPath of files) {
    ...
    const fm = extractFrontmatter(content);
    if (!fm) { errors.push(`steering/${relPath}: missing or invalid frontmatter`); continue; }
...
function validateBundleSchemas(repoRoot) {
  ...
  for (const file of files) {
    ...
    if (!bundle) { errors.push(`bundles/${file}: invalid YAML`); continue; }
  ...
  for (const file of files) {
    ...
    if (agent.domain) {
      ...
      if (!existsSync(domainDir)) {
        errors.push(`${file}: domain '${agent.domain}' has no matching steering/${agent.domain}/ directory`);
      }
    }
  }
  return errors;
...
function validateBundles(repoRoot) {
  ...
  for (const name of bundles) {
    try {
      ...
      errors.push(`bundles/${name}.yaml: ${err.message}`);
    }
  }
  return errors;
}
