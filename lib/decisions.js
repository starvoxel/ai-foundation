...
// `Last Amended` is also deliberately excluded: it only appears on records
// that have been through the amendment ladder (AIF-META-002), and all 16
// current records have never been amended. Requiring it here would fail
// every existing record at once (Epic AIF-003 Risk 11).
...
/**
 * Count the data rows of a record's optional "## Amendments" table.
 * Returns 0 when the section is absent, or when it is present but malformed
 * (heading with no table) — a malformed Amendments table is non-fatal per
 * Epic AIF-003 Section 5; only a missing `Decision ID`/`Status` aborts the
 * parse. Rejected amendment rows are counted too: this is a count of "how
 * many times was this record proposed for amendment", not "how many
 * succeeded" (AIF-META-002).
 *
 * "## Errata" is deliberately never read here or anywhere else in this file
 * — an indexed errata list would imply readers should consult it, which
 * contradicts the rung's defining property (AIF-META-002).
 *
 * Known limitation: like `parseMetadataTable`, this does not track fenced
 * code blocks, so a literal "## Amendments" heading quoted inside a fenced
 * example (e.g. in a spec/template document) would be miscounted. Accepted
 * per Epic AIF-003 Risk 1 — these headings only appear in template/spec
 * documents, not in records under `paths.decisions`.
 * @param {string} content - Raw file content
 * @returns {number}
 */
function countAmendmentRows(content) {
  const lines = content.split(/\r?\n/);
  const headingIdx = lines.findIndex((line) => line.trim() === '## Amendments');
  ...
  // Header row, then a "|---|---|---|---|"-style separator row (any number
  // of columns — the Amendments table has more columns than Metadata's).
  i++;
 ...
 * @returns {{ record: { id: string, tier: string|null, domain: string|null, title: string,
 *   status: string, path: string, references: string[], supersedes: string[], tags: string[],
 *   last_amended: string|null, amendment_count: number } }
 *   | { error: string }}
 */
export function parseDecisionRecord(content, relPath) {
  ...
  return {
    record: {
      ...
      last_amended: fields['Last Amended'] && fields['Last Amended'] !== '—'
        ? fields['Last Amended']
        : null,
...
export function buildDecisionIndex(records) {
  const entries = records.map((record) => ({
    ...
    referenced_by: [],
    tags: record.tags || [],
    last_amended: record.last_amended ?? null,
