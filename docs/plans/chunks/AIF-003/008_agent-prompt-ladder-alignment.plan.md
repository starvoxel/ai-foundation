...
|---|---|---|---|---|
| 1 | **`agents/ai-engineer.yaml`'s `skills:` list includes `skill/ai-engineering-plan` but neither `skill/chunk-planning` nor `skill/plan-lifecycle`, despite AI-Engineer owning 6 of this Epic's 8 chunks and therefore authoring and following Chunk Plans (including this one).** This looks like a genuine agent/skill cross-reference gap, discovered while reading this chunk's required inputs. It has now been independently flagged twice: once by this chunk's own rev 1 draft, and separately by AIF-003-003's chunk-planning agent while it read the same dependency files for its own plan — two independent authors converging on the same finding strengthens the case that this is a real gap rather than an artifact of one plan's reading of the files. | Question | M | Not in this chunk's file ownership (`chunks.json` names only `agents/architect.yaml`, `agents/engineering-manager.yaml` for `008`) or in the Epic's Scope section — fixing it here would be undirected scope expansion (engineering-core Rule 4). Raised here and reported to the human/Tech-Lead for a decision on whether it becomes a follow-up chunk, a standalone Tier 1/2 AI-Engineer task, or is deferred. Not implemented as part of AIF-003-008. |
