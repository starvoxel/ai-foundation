...
- Defining or adding a `Supersedes`/`Superseded By` field to the decision-record/decision-brief templates — neither chunk 002 nor 003 currently include this field explicitly in their Metadata table (only `Referenced By`/`References`); see Section 13, Risk 3, for how this chunk handles that gap without silently inventing new template scope.
...
4. **Decision**: Malformed or unparseable `.decision.md` files (missing required Metadata fields, malformed table) cause the whole `-d` run to fail loudly (non-zero exit, clear error naming the offending file) rather than being silently skipped.
   **Rationale**: Direct implementation of AIF-002 Epic Plan Section 4's Error States table ("surface any parse failure... to the human rather than guessing or dropping the offending record silently"). A silently-dropped record would produce an incomplete index that looks valid, which is worse than a loud failure.
...
- [ ] No `WebSearch`/`WebFetch` or network calls anywhere in this chunk — local, deterministic, file-system-only, consistent with AIF-002 Epic Plan Section 6.
...
- [ ] All tests in Section 10 pass (`tests/unit/decisions.test.js`, `tests/integration/decisions-index.test.js`)
