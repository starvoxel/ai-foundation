...
function installBundle(bundleName, harness, repoRoot) {
  ...
  // Claude-only: ensure the shared blocked_commands hook script is present.
  // Unconditional and idempotent — not bundle-owned, no manifest entry, so
  // no single bundle's uninstall can remove it out from under another.
  if (harness === 'claude' && typeof adapter.installHookScript === 'function') {
    adapter.installHookScript();
  }
