/**
 * config command — deterministic .aiconfig.json field resolution.
 *
 ...
 *
 * Usage:
 *   aif config <key>          — print the resolved value (config value,
 *                                else its documented default)
 *   aif config <key> --abs    — print an absolute path (for paths.* keys)
 */
 ...
 */
export function runConfig(parsed, cwd) {
  const key = parsed.positional[0];
  if (!key) {
    console.error('Usage: aif config <key>');
    return 1;
  }
