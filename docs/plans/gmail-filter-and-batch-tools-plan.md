...
4. **Implement I/O wrappers in `logic.js`** (same pattern as existing tools — take `gmail` client as first arg):
   ...
   - `senderReport(gmail, { query, max_senders })` — pages `users.messages.list` for matching IDs, then fetches `From`/`Subject` for each via `users.messages.get` with `format: 'metadata', metadataHeaders: ['From', 'Subject']` (headers only — never a body), batched using the Gmail API's HTTP batch endpoint (up to 100 sub-requests per batch call) to keep round-trips proportional to hundreds, not thousands, of calls for a 2-year window. Passes the extracted `{ from, subject }` pairs to `aggregateBySender`.
