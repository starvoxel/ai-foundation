...
export function createGmailServer(gmailOrFactory) {
  ...
  /** @param {unknown} result @returns {{ content: { type: 'text', text: string }[] }} */
  const asText = (result) => ({
    content: [{ type: 'text', text: JSON.stringify(result, null, 2) }],
  });
  /** @param {Error} err @returns {{ content: { type: 'text', text: string }[], isError: true }} */
  const asError = (err) => ({
    content: [{ type: 'text', text: JSON.stringify({ error: err.message }, null, 2) }],
    isError: true,
  });
  /**
   * @param {(gmail: import('googleapis').gmail_v1.Gmail, args: any) => Promise<unknown>} fn
   * @returns {(args: any) => Promise<{ content: { type: 'text', text: string }[], isError?: true }>}
   */
  ...
  server.registerTool(
    ...
    {
      description:
        'Lists/searches messages matching a Gmail search query, with optional label filter and pagination.',
      inputSchema: {
        query: z.string().optional().describe('Gmail search query'),
        label_ids: z
          .array(z.string())
          .optional()
          .describe('Restrict to messages having all of these label IDs'),
  ...
  server.registerTool(
    ...
    {
      description:
        'Lists all existing Gmail filters. Check before creating a new one to avoid duplicates.',
      inputSchema: {},
    },
    wrap(listFilters),
  );
  ...
  server.registerTool(
    ...
    {
      description:
        "Convenience/discovery tool, not a dependency of any other tool: aggregates messages matching a query by sender (From/Subject metadata only, never bodies). If removed, the same result is obtainable via gmail-list-messages + gmail-get-message(format:'metadata') per message.",
      inputSchema: {
        query: z
          .string()
          .optional()
          .describe('Gmail search query restricting which messages to aggregate'),
        max_senders: z
          .number()
          .optional()
          .describe('Maximum number of senders to return, ranked by count descending'),
      },
    },
    wrap(senderReport),
  );
  ...
  server.registerTool(
    ...
    {
      description:
        'Adds and/or removes labels on a message (archive, mark read/unread, star). Reversible.',
      inputSchema: {
        message_id: z.string().describe('Gmail message ID'),
  ...
  server.registerTool(
    ...
    {
      description:
        'Moves a message to Trash. Recoverable for 30 days — not gated (see gmail-delete-message for permanent deletion).',
  ...
  server.registerTool(
    'gmail-create-label',
    {
      ...
      inputSchema: {
        name: z.string().describe('Label name, e.g. "Project X" or nested "Project X/Invoices"'),
      },
    },
    wrap(createLabel),
  );
  ...
  server.registerTool(
    ...
    {
      description:
        'Creates a Gmail filter (auto-apply-label rule). Only affects future mail — use gmail-batch-modify-labels to backfill history. Additive, reversible via gmail-delete-filter.',
      inputSchema: {
        ...
        negated_query: z
          .string()
          .optional()
          .describe('Raw Gmail search string that must NOT match'),
        ...
        size: z
          .number()
          .optional()
          .describe('Match messages by size in bytes, used with size_comparison'),
        size_comparison: z
          .enum(['larger', 'smaller', 'unspecified'])
          .optional()
          .describe('Paired with size'),
        add_label_ids: z
          .array(z.string())
          .optional()
          .describe('Label IDs to apply to matching messages'),
        remove_label_ids: z
          .array(z.string())
          .optional()
  ...
  server.registerTool(
    ...
    {
      description:
        'Deletes a Gmail filter (the rule only, not labels already applied). Not gated — trivially recreatable via gmail-create-filter.',
  ...
  server.registerTool(
    ...
    {
      description:
        "Adds and/or removes labels across many messages in one call, chunking internally at the API's 1000-message limit. Reversible.",
      inputSchema: {
  ...
  server.registerTool(
    'gmail-reply-message',
    {
      description:
        IRREVERSIBLE_NOTICE +
        'Sends a reply within an existing thread, quoting the original message.',
      inputSchema: {
        ...
        reply_all: z
          .boolean()
          .optional()
          .describe('Reply to all original recipients instead of just the sender'),
      },
    },
    wrap(replyMessage),
  );
  ...
  server.registerTool(
    ...
    {
      description:
        IRREVERSIBLE_NOTICE + 'Permanently deletes a message, bypassing Trash. Cannot be undone.',
      inputSchema: { message_id: z.string().describe('Gmail message ID') },
    },
    wrap(deleteMessage),
  );
  ...
  server.registerTool(
    'gmail-delete-label',
    {
      description:
        IRREVERSIBLE_NOTICE +
        'Permanently deletes a user label and removes it from every message it is applied to. Cannot be undone.',
