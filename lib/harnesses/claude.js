 ...
 * hook script, installed once per machine via installHookScript().
 */
const HOOK_SCRIPT_SRC_DIR = join(__dirname, 'assets', 'block-command');
...
function hookScriptPath() {
  return join(TARGETS.scripts, 'block-command', 'cli.js');
}
 ...
 * ~/.claude/scripts/block-command/. Called unconditionally on every claude
 * harness install run, regardless of which bundle triggered it — the
 * script is not bundle-owned, has no manifest entry, and is never removed
 * by `aif uninstall` for any single bundle.
 * @returns {void}
 */
export function installHookScript() {
  const targetDir = join(TARGETS.scripts, 'block-command');
  for (const file of ['logic.js', 'cli.js']) {
    const src = join(HOOK_SCRIPT_SRC_DIR, file);
    const content = readFileSync(src);
    writeToTarget(join(targetDir, file), content);
  }
  console.log('  ✓ hook script: block-command');
}
...
function installMcpStdio(name, serverDef, repoRoot) {
  ...
  if (existsSync(targetPkgJson)) {
    try {
      execSync('npm install --omit=dev', { cwd: serverTargetDir, stdio: 'pipe', shell: true });
    } catch (err) {
