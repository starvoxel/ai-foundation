 ...
 * @property {(agent: AgentDef) => string|object} transformAgent - Agent YAML → output content
 ...
 * @property {Record<string, (serverName: string, serverDef: ServerDef, repoRoot: string) => Array<{path: string, hash: string}>>} [serverInstallers] - Protocol → installer function map
 * @property {(agent: AgentDef) => string|null} [detectSharedResource] - Given a parsed agent, returns the name of a harness-level shared resource it requires (e.g. "block-command"), or null
