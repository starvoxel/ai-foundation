 ...
 *   --standards <list>    Comma-separated standards to load (e.g. typescript-node,api-design)
...
import { validateProjectName, applyProjectConfig, buildAiConfig } from '../project-init.js';
...
function configFromArgs(args) {
  ...
  if (typeof args.standards === 'string') {
    config.standards = args.standards.split(',').map(s => s.trim()).filter(Boolean);
  }
...
export async function promptForConfig(config, options = {}) {
  ...
  const rl = options.rl || createInterface({
    input: process.stdin,
    output: process.stdout,
  });

  try {
    config.projectName = await askField(
      rl, 'Project name', config.projectName, skipIfProvided
    );

    config.organization = await askField(
      rl, 'Organization', config.organization, skipIfProvided
    );

    config.language = await askField(
      rl, 'Language (e.g. typescript, csharp, python, go, rust)', config.language, skipIfProvided
    );

    config.moduleId = await askField(
      rl, 'Module ID (npm scope, Go module, crate, namespace)', config.moduleId, skipIfProvided
    );

    config.repository = await askField(
      rl, 'Repository URL', config.repository, skipIfProvided
    );

    if (!skipIfProvided || !config.standards) {
      const currentStd = config.standards ? config.standards.join(', ') : '';
      const stdAnswer = await askField(
        rl, 'Standards (comma-separated, blank to skip)', currentStd, false
      );
      if (stdAnswer) {
        config.standards = stdAnswer.split(',').map(s => s.trim()).filter(Boolean);
      }
    }
  } finally {
    if (!options.rl) {
