...
| `decisions.js`                        | Parses a `.decision.md` file's `## Metadata` table, builds/diffs the decisions index, inverts `Supersedes` into `superseded_by`. The parser reads that Markdown table, not YAML frontmatter, until `docs/process-model.md`'s check 30 retargets it to MADR frontmatter. | `parseDecisionRecord()`, `buildDecisionIndex()`, `diffDecisionIndex()`                                     |
