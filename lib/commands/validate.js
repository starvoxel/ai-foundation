...
const CITATION_FAMILIES = [
  ...
  {
    // `steering/{path}.md` → that file's `### Rule N: Name` headings
    refRe: /steering\/[a-z0-9/_.-]+\.md/g,
    headingRe: /^###\s*Rule\s+\d+:\s*(.+?)\s*$/gm,
    headingLineRe: /^\s*###\s*Rule\s+\d/,
...
export function validateCitations(repoRoot) {
  ...
  const targets = [
    ...collectFilesFlat(join(repoRoot, SOURCE_DIRS.skills), ['.md', '.yaml'], SOURCE_DIRS.skills),
    ...collectFilesFlat(join(repoRoot, SOURCE_DIRS.agents), ['.yaml', '.md'], SOURCE_DIRS.agents),
    ...collectFilesFlat(join(repoRoot, SOURCE_DIRS.steering), ['.md'], SOURCE_DIRS.steering),
  ];
...
function collectFilesFlat(dir, extensions, prefix = '') {
  const results = [];
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      results.push(...collectFilesFlat(join(dir, entry.name), extensions, rel));
