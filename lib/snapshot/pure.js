...
export function isRuntimeFile(relPath) {
  return !relPath.startsWith('tests/') && !relPath.endsWith('.test.js') && relPath !== 'snapshot.json';
}
 ...
 * (that requires listing directories, which is the caller's job when this
 * function returns an empty target list).
 *
 ...
 * @returns {{ ok: true, targets: Array<{ kind: string, name: string }> } | { ok: false, error: string }}
 */
