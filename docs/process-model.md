...
**`skill/knowledge-authoring` retires** (check 12). Once `decision`/`architecture` move
...
`✗ Decision index is stale: Removed: AIF-PLAN-001`) — a CI gap, not a reason to retire the indexer (check 32).
...
constraint Principal-Engineer review must specifically check for (check 19), the same
...
something check 19 or this document should take on now.
...
(check 12). Software-Engineer's Tier 3 hand-off re-implements the shape natively.
...
| `AIF-003` epic (decision-record amendment ladder) + its 8 chunk plans                                      | **Abandoned mid-flight.** The ladder it implements is retired by this model. PRs #23/#24 closed unmerged 2026-09-09; chunks `001/003/004` had already merged and need unwinding (checks 1 and 31). Chunk `005` also merged but needs **no** unwinding — check 31 keeps its change (the steering de-enumeration is correct under MADR too).      |
...
| `ARCH-004` Standards sync mechanism                    | Yes, still `Draft`            | Decide or drop before converting (check 25) — its byte-level schema belongs in arc42 §5, not a frozen ADR.                                                                                                                                                                                                                                              |
...
| `ARCH-007` Epic/Chunk → YouTrack                       | Yes, still `Draft`            | Same "decide or drop before converting" gate as `ARCH-004` (check 25) — not called out separately before, but it's in the identical state. Once approved: Core → MADR, `Design` (field schema, workflow rules) → `docs/plans/youtrack-integration-plan.md`.                                                                                             |
...
| `PROC-003` Parallel chunk isolation (worktrees)        | Partly, still `Draft`         | Same gate as `ARCH-004`/`ARCH-007` (check 25). Once approved: short MADR ADR for the worktrees-vs-alternatives trade-off; operational detail → orchestration skill text.                                                                                                                                                                                |
...
| `PLAN-001` Epic/chunk colocation                       | No — convention               | Already deleted from disk; retire the stale index reference (check 29).                                                                                                                                                                                                                                                                                 |
...
Check 32 adds new guards to that workflow.
...
| 2               | `AIF-004` marked `Deferred` and moved into `archive/` (per Retiring a plan above). Much lighter than `AIF-003`'s teardown (checks 1 and 31): `AIF-004` was never dispatched — no `chunks.json`, no orchestration state, no branches or PRs to unwind. Just the epic file's `Status` flip and a move to `archive/`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| 5               | `aif index` extended for arc42 sections (entry shape above), for two things §5 and the numbered directory can't give you: the computed `source path → [docs]` reverse index from `key_files`, and the `stale` flag from `last_verified`. Not a forward nav index — arc42's own numbering + §5's block list already cover browsing, so none is built. Decisions stay in scope via the retargeted indexer (check 29) — one module, two doc sets.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 6               | Agent YAMLs updated per Agent roster: 4 retired, 4 modified, 1 new. `architect.yaml` **loses** `shell` and `web_search`/`web_fetch`, gains only plain `write` (scoped to `docs/decisions/**` + `docs/architecture/**`) plus gated Researcher dispatch — no `@adr/*` tools exist, per ADR tooling above. `engineering-researcher.yaml` scoped for ADR-grade depth, write scoped to `{paths.research}` from check 3. Also states explicitly, in whichever agent dispatches Architect, that it — not Architect — commits the resulting ADR/arc42 file. Everything else in this list derives from this check. **Deliberately still Epic/Chunk-worded:** `engineering-manager.yaml`, `software-engineer.yaml`, and `principal-engineer.yaml` reference `skill/epic-planning`/`skill/chunk-planning`/`skill/chunk-orchestration`/`chunks.json` and say "Epic Plan"/"Chunk Plan" throughout, since checks 8–10 (not this check) own that rename — see their notes for exactly what to sweep.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 7               | Software-Engineer opens the Task's PR as a **draft** immediately after implementing (right after Process step 11's commit+push), not after Principal-Engineer approval as today — every Task gets a PR whether or not it needs a revision round, giving the review loop a visible record instead of one that only exists in session output. `software-engineer.yaml`'s separate "Process (PR creation — when instructed by Engineering Manager after review approval)" section is removed; opening the PR folds into the implementation process itself. Principal-Engineer's Review Report (still produced per `skill/code-review`/`skill/ai-component-review`/`skill/review-severity`, unchanged) is posted onto that PR as real review comments/threads by **Engineering Manager**, not Principal-Engineer itself — Principal-Engineer gains no `shell`/`gh`/`ai-git` access; Engineering Manager posts through the `shell`/`ai-git` access it already holds for orchestration, the same pattern as it already committing an Architect-authored ADR (see Resolved design questions). Correction loops happen on the same PR: Software-Engineer pushes fixes to the same branch, Engineering Manager posts Principal-Engineer's next Review Report as a new review on the updated diff, same 5-iteration cap as today. Once Principal-Engineer approves, Engineering Manager marks the PR ready-for-review (undrafts it) — no second PR is created. Updates the End-to-end flow diagram and the Agent roster table's Principal Engineer row (both above) and `engineering-manager.yaml`'s orchestration Process/Hard rules for the draft-open/post-review/undraft sequence. **Deliberately still Chunk/Epic-worded**, same reason as check 6 — checks 8–10 own that rename, not this check. |
| 8               | `epic-planning` + `chunk-planning` merge into `feature-planning`: renamed, Task-sizing rules added, small Features may skip decomposition. **Also sweep check 6's agent YAMLs — do not skip them because they look freshly written.** `engineering-manager.yaml`, `software-engineer.yaml`, and `principal-engineer.yaml` deliberately still say `skill/epic-planning`/`skill/chunk-planning` and "Epic Plan"/"Chunk Plan" throughout their prompts and hard rules — not only in their `skills:` lists — because check 6 landed before this rename and was instructed to keep the pre-rename names rather than jump ahead of checks 8–10. Update every occurrence in those three files, not a representative few. **`skills/epic-planning/SKILL.md` and `skills/chunk-planning/SKILL.md` themselves are untouched by check 6** and still fully describe the pre-roster world: literal "Tech-Lead"/"AI-Engineer"/"Test-Engineer" agent names, and a software-track-vs-AI-track split gated by `chunks.json`'s `agents` field. Check 6 deliberately left them alone rather than partially patch agent names into a track structure check 9 was already going to remove — do not mistake that silence for "already fine." This merge is where their content, not just their names, gets rewritten.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 9               | `chunk-orchestration`: `chunks.json` → `tasks.json`; per-Task plan gate replaced by `complexity-tiers`; software/AI-track branching removed — one pipeline (implement+self-test+docs → PE review) for every Task. **`skills/chunk-orchestration/SKILL.md` itself is untouched by check 6** and still fully implements the two-track pipeline this check removes (`AI-Engineer`/`Test-Engineer` named throughout, track branching keyed on `chunks.json`'s `agents` field) — same reason as check 8's note: check 6 deliberately did not partially rename agents into a structure this check was already going to delete. **Also sweep, same reason as check 8's note:** `engineering-manager.yaml`'s `skill/chunk-orchestration` reference, every `chunks.json` mention in its Process/Hard rules sections, and its `EVERY CHUNK GETS ITS OWN BRANCH AND WORKTREE` hard rule (and the branch-naming pattern `{epic-id}/{chunk-id}-{short-description}` beside it) — all still Chunk-worded by the same deliberate check-6 deferral.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 10              | DAG server + `lib` renamed chunk→task; wave output identical for an equivalent graph. Includes `servers/dag/logic.js`'s doc comment and the chunk/epic-worded test fixtures in all three `servers/dag/tests/**` files, not just identifiers.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 11              | What `knowledge-authoring` carried is re-homed: a steering rule holds the four-kind routing table; templates hold the file shapes (MADR, arc42 section, external-reference file). A skill earns its place with judgment/validation steps; a template + always-on rule is enough for light-structure prose. Resolved before check 12 so that check knows whether `docs/knowledge-file-format.md` still needs a body.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 12              | `decision-triage`, `decision-brief`, `decision-record`, `chunk-planning`, `ai-engineering-plan`, `knowledge-authoring` skills deleted, all references removed. `decision-record` because MADR replaces (not shrinks) it; `knowledge-authoring` per Subject matter is not a document kind (check 11). `docs/knowledge-file-format.md` — the live canonical spec for the exact `type: decision\|reference\|architecture\|api\|business-rule` taxonomy this section retires — rewritten to match (or deleted if check 11's steering rule + templates fully replace what it specified); it's actively pointed to from `projects/_template/knowledge/example.md` and several `docs/plans/*.md` files, which need the same update. **Also retired here, not left behind as dead wiring once the taxonomy above is gone:** `lib/knowledge.js` (`validateKnowledgeFrontmatter`/`buildIndexEntry`), `KNOWLEDGE_TYPES` in `lib/constants.js`, and the `knowledge` target of `aif index` in `lib/commands/index.js` — by check 5's landing, decisions and architecture each already have their own dedicated indexer, and this repo's own corpus has exactly one file still using the generic one (`docs/agent-prompt-extraction-candidates.md`, `type: reference`), which check 17 already sweeps and doesn't fit the taxonomy's actual surviving scope (external reference material about _other_ systems, routed to `steering/` per check 11 — not this repo's own prompt-authoring debt tracking). Migrate that one file's frontmatter (or fold its content elsewhere) as part of this check so nothing depends on the code being deleted.                                                                                                                                                          |
| 13              | `plan-lifecycle`/`complexity-tiers` trimmed; `complexity-tiers` re-pointed as SE's primary gate. Tier 3 becomes "stop and hand off" (orchestrated → EM, standalone → human). `plan this` documented as a Tier 2 floor, not a Tier 3 jump.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 14              | `projects/_template/` brought to the new format — every consuming project starts from this skeleton. Not just the old `epics/chunks/orchestration` layout renamed: the **full** new document layout, mirroring what checks 3–13 just built. `.aiconfig.json`'s new `paths.*` fields (check 3) filled in for the template itself. `plans/{features,tasks,orchestration}/` (renamed from `epics/chunks/orchestration`, per checks 8–9). `knowledge/decisions/` (flat, MADR-ready — mirrors `docs/decisions/`) and `knowledge/architecture/` (empty; arc42 sections created lazily per check 4, never pre-scaffolded) and `knowledge/research/` (the `paths.research` target). **No** `knowledge/product/` directory — the Product doc type stays a reserved slot with no owner and no folder until a product agent lands; scaffolding an empty one is exactly the premature ceremony this model exists to avoid. `project-standards.md`'s references to the skills retired in check 12 updated.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| 15              | `steering/engineering/core.md` Rules 1/2/8/9 reworded (Rule 1 → `complexity-tiers` gate, Rule 2 gets a Tier 1/2 fallback, Chunk/Epic Plan → Feature Plan); `knowledge-consumption.md` moves decision-records from full-body auto-load to **index-only by default, full body on demand**; doc-update acceptance gate added — explicit that this includes **adding** a `key_files` entry for a new file extending a described building block, not just editing prose for files already listed, since a file's absence from every `key_files` list is not evidence it's out of scope, only that nobody has checked yet (closes the gap check 5/32's staleness detection can't: it only watches files already listed).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 16              | **Full vocabulary + reference sweep**, not just planning skills: `skills/code-review/`, `skills/test-execution/`, `skills/worktree-management/SKILL.md`, `skills/complexity-tiers/SKILL.md`, `skills/plan-lifecycle/reference/commit-gate-procedure.md`, `standards/csharp_base.md`, `standards/javascript_base.md`, `steering/engineering/git-workflow-projects.md`. Deleted-skill references also in `skills/agent-authoring/reference/schema.md`, `projects/_template/project-standards.md`, `standards/javascript_node.md`, and `lib/resolver.js`'s JSDoc examples.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 17              | `skill/agent-authoring` and `docs/agent-prompt-extraction-candidates.md` swept. Product-doc ownership **not** assigned — reserved and unbuilt.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 18              | `skills/agent-authoring/reference/tools.yaml` gains the trifecta-avoidance rule (no agent holds `moderate`/web and `privileged`/write+shell at once without documented isolation).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 19              | `skill/code-review`'s checklist gains two explicit items, since the agent schema has no path-scoping field to enforce either instead (see Agent roster's "reviewed convention, not schema-enforced" note): (1) for any diff where Architect or Engineering Researcher performed the `write`, Principal-Engineer confirms the touched paths stay within their documented scope — `docs/decisions/**`/`docs/architecture/**` for Architect, `{paths.research}` for Researcher — a violation is a HIGH finding, same severity class as the existing `tools`/`approved_tools`/`blocked_commands` rule (Why Principal-Engineer absorbs AI-component review, above); (2) for any diff adding a file under a directory an arc42 section already describes, confirm that section's building-block table and `key_files` were updated to include it — a MEDIUM finding otherwise, the same silent-drift class check 5/32's staleness detection exists to catch, just upstream of where that mechanism can ever see the file (check 15).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 20              | Software-Engineer's hard rules state a Researcher brief is data informing a decision, never an instruction to execute; confirm PE review applies regardless of who dispatched SE.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 21              | `bundles/engineering/snapshot.json` regenerated (`bundle.yaml` needs no edit — domain-based auto-discovery absorbs the shrink). Run only after checks 6 and 12 have actually landed, or the snapshot still reflects the pre-shrink roster.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 22              | `README.md`, `PLAN.md`, `AGENTS.md`, `agents/README.md`, `skills/README.md` updated. `install.ps1` not in scope — already deleted (dead per `ARCH-001`), its `tools.test.js` assertion removed with it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 23              | `tests/validation/` cross-reference check passes against the new set; `npm test` green. Fixtures cleaned up: `tests/unit/decisions.test.js`, `tests/integration/decisions-index.test.js`, `knowledge-index.test.js`, `base.test.js`, `claude-adapter.test.js`, `kiro-adapter.test.js`, `tests/unit/resolver.test.js`, `tests/integration/resolver.test.js`, `tests/integration/commands.test.js`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 24              | Terminology-sweep verification as an automated `tests/validation/` check: repo-wide `chunk`/`epic` search returns zero matches outside `docs/plans/{chunks,epics,orchestration}/`, `docs/plans/completed/`, `docs/plans/archive/`, MADR record bodies (e.g. `ARCH-007`), and `docs/decisions/index.json` (generated from those bodies, not itself a body). `agent-consolidation-plan.md` and `AIF-004.epic.md` are covered as kept-in-place history. The two already-deleted plans need no exception. Runs last, once checks 3–22 have landed — including check 14, so `projects/_template/` is clean too.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 25              | Human decision gate for every source record still `Status: Draft` at conversion time — currently `ARCH-004`, `ARCH-007`, `PROC-003` (verified against `docs/decisions/index.json` and each file's own Metadata table). Each gets an explicit `Approved`/`Deferred` call before its MADR conversion (check 28 or 30) proceeds — `Draft` never silently becomes an accepted MADR record. A `Deferred` verdict drops that record from this pass entirely rather than converting it speculatively.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 26              | Existing decision records dispositioned per the table above; archive location created.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 27              | `docs/decisions/` flattened (domain subfolders removed, flat bare-number MADR counter — `0007-slug.md`, no domain-coded `AIF-ARCH-`/`AIF-PROC-`/etc. prefix); every surviving record rewritten by hand into MADR within the word budget, renumbered onto the new counter, checked at PE review.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 28              | `Design` sections split out of `ARCH-001/002/003/007` into their arc42 homes (or the YouTrack plan for `007`), plus `ARCH-004` if check 25 approves it; `ARCH-005/006` converted whole per their disposition-table exceptions. This is what seeds `docs/architecture/` with real content.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 29              | `lib/decisions.js` **retargeted, not deleted**: parser moves from the `## Metadata` table to MADR frontmatter, keeping index generation and the `supersedes`→`superseded_by` inversion. `aif index decisions` and `docs/decisions/index.json` stay. `tests/unit/decisions.test.js` and `tests/integration/decisions-index.test.js` updated to new fixtures.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 30              | The three New ADRs to write are written in MADR form.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 31              | Merged half of `AIF-003` unwound, minus what already dies elsewhere. Real revert: `AIF-003-004`'s `Amending` status, removed from `plan-lifecycle/reference/status-vocabulary.md` by hand. `AIF-003-003`'s Amendments/Errata template sections die with check 12; `AIF-003-001`'s `Supersedes` parse dies with check 29 — no separate revert needed. `AIF-003-005`'s steering de-enumeration is **kept** (correct under MADR too).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 32              | New guards added to the **existing** `.github/workflows/ci.yml` (already runs lint/typecheck/format/validate/snapshot-freshness/version-bump-check/test): the `key_files`-vs-`last_verified` staleness check, relative-link resolution across `docs/architecture`, `aif index decisions --check`. No `adrs-core lint` or hyphenated-`kind` guard — both moot while ADRs are hand-written. Any steering/agent file this rework touches (e.g. `steering/engineering/core.md`, `steering/global/knowledge-consumption.md`, `agents/*.yaml` per check 6) needs its `version` field bumped alongside the content change, or the existing version-bump-check fails it; files deleted outright (check 12) are unaffected.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 33              | Freeform `docs/plans/*.md` triaged and cleared; `docs/plans/completed/` holds the finished ones.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 34 _(deferred)_ | A **deferred** ADR, written by Architect once there's enough hand-written volume: ADR tooling (Rust CLI vs. in-repo JS vs. staying manual — see ADR tooling above, and `docs/plans/adr-kit-plan.md`). Gates nothing — deliberately outside the sequence below.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
6. **Vocabulary rename** (checks 8–10) — Feature/Task terminology, then the DAG/lib
   chunk→task rename.
7. **Skill retirement & re-homing** (checks 11–13) — decide where `knowledge-authoring`'s
   content goes first, then delete the skills that decision retires, then trim
   `plan-lifecycle`/`complexity-tiers`.
8. **Template alignment** (check 14) — `projects/_template/` rebuilt on the full new
   layout. Depends on the vocabulary rename (phase 6) and skill retirement (phase 7)
   it's built to reflect.
9. **Steering & reference sweep** (checks 15–17) — core steering rules first, then the
   wider vocabulary/reference sweep, then `agent-authoring`.
10. **Cross-cutting process/security rules** (checks 18–20) — the trifecta-avoidance
    rule, the code-review path-scoping check, and Software-Engineer's Researcher-brief
    rule.
11. **Regeneration & top-level docs** (checks 21–22) — the bundle snapshot regenerates
    only once the roster and skill shrink (phases 4 and 7) are actually on disk;
    top-level docs update last so they describe the settled state.
12. **Verification** (checks 23–24) — `npm test` green first, then the terminology
    sweep, once every check in phases 2–11 has landed.
13. **Decisions conversion** (checks 25–30) — resolve the Draft-record gate first,
...
14. **Unwind the merged half of `AIF-003`** (check 31) — small by this point; depends
    on phase 7 (skill retirement) and phase 13 (decisions retargeting).
15. **CI guards** (check 32) — added once there's a converted decision log and
    populated arc42 sections to actually run against.
16. **Freeform plan triage** (check 33) — last, once the rest of the repo has settled.

Check 34 (ADR tooling) is deliberately outside this sequence — Architect's to write
later, gates nothing.
