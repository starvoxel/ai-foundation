...
/**
 * Resolve a project's knowledge directory via the shared .aiconfig.json
 * resolver (lib/aiconfig.js): the configured paths.knowledge value, or its
 * documented default.
 * @param {string} projectRoot
 * @returns {string} Absolute path to the knowledge directory
 */
function resolveKnowledgePath(projectRoot) {
  return getConfigPath(projectRoot, 'paths.knowledge');
}

/**
 * Resolve a project's decisions directory via the shared .aiconfig.json
 * resolver. paths.decisions' own default derives from the *resolved*
 * paths.knowledge value (see lib/aiconfig-defaults.js), so a configured
 * paths.knowledge override still shifts the decisions default with it.
 * @param {string} projectRoot
