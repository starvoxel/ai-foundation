...
- **PRDs are an input, not a fifth type.** The Feature Plan's goal/acceptance-criteria
  section _is_ the PRD today. PRDs move to the reserved slot once a product agent exists.
...
**`skill/knowledge-authoring` retires** (check 13). Once `decision`/`architecture` move
out and `api`/`business-rule` become tags, only reference material about _other_
...
| Process / tooling / convention / ownership change                        | No record — edit the skill/steering/agent file; the commit is the record |
...
**No blanket CHANGELOG requirement either, same reasoning.** Software-Engineer's prompt
has carried "a CHANGELOG entry is mandatory for every completed Chunk Plan" since before
this rework, keyed to `## [{date}] {Plan ID} — {Title}`. No `CHANGELOG.md` exists
anywhere in this repo — not at the root, not in `projects/_template/` — and no other
agent, skill, or steering file references the concept at all: the rule has been
unenforceable dead process the whole time, not a mechanism this rework can just rename
`Chunk Plan` to `Task` in and call fixed. It also contradicts this document's own Goal
(git holds history; agent context holds only what's binding and current) — a
CHANGELOG.md is a manually-maintained shadow of git history, the same accretion pattern
that killed the old decision-record ladder above. Dropped as a framework-level
Software-Engineer requirement (check 9); kept as something a project can opt into via
its own `project-standards.md` if it actually ships versioned releases where a
human-facing CHANGELOG earns its keep — Software-Engineer already must follow whatever
that file specifies, so no new conditional logic is needed in its own prompt.

### ADR format — MADR

Standard MADR, replacing the in-house options-exploration template. Records _that_ a
decision was made and _why_, not the research trail.
...
**Litmus test:** _how the system works_ vs. _why this path won_ — only the second
...
`✗ Decision index is stale: Removed: AIF-PLAN-001`) — a CI gap, not a reason to retire the indexer (check 33).
...
published` is the _only_ value an author writes (is the doc still being written?) —
...
1. **Reverse index** — `key_files` inverted: `aif index` builds `source path → [docs]`,
   ...
   `supersedes`).
2. **Real staleness detection** — `last_verified` as a commit SHA means _has any
   `key_files` entry changed since?_ is mechanical, turning doc drift into a CI
...
| **Engineering Researcher** _(new)_ | Web research → decision-ready brief, for Architect, EM, or SE. Scoped for ADR-grade depth, not just light briefs.                                                                                                             | Yes, scoped to `{paths.research}` (default `knowledge/research/`, new `.aiconfig.json` field — check 3), `.md` only | **No**                    | Yes    | No                                                                                           |
| **Principal Engineer**             | Review gate — standards + the Feature's outline. Checklist branches by artifact type. Findings delivered as PR review comments, posted by Engineering Manager on its behalf (check 7) — Principal Engineer gains no new tool. | No (findings only)                                                                                                  | No                        | No     | No                                                                                           |
...
`tools:`/`approved_tools:` is a flat list of tool _names_, nothing narrower. Dropping
`shell`/`web_search`/`web_fetch` from Architect's `tools:` list entirely _is_ a real,
...
constraint Principal-Engineer review must specifically check for (check 20), the same
...
something check 20 or this document should take on now.
...
Engineering Manager -- spots an ADR-worthy fork? --> Architect (subagent, gated)
  ...
  Engineering Manager posts the Review Report as PR review comments (PE holds no shell/gh)
      |
...
multi-agent chain — the old model's largest coordination cost. The PR itself now opens
before that loop starts, not after it (check 7) — see Resolved design questions below
for who posts Principal-Engineer's findings onto it.

### Resolved design questions

**How detailed should a Feature Plan be?** Coarse — _what_, not _how_: acceptance
...
orchestrated/standalone branching _shape_ even though that skill retires — the pattern
...
**Who posts Principal-Engineer's Review Report onto the PR, given Principal-Engineer
holds no `shell`?** Same answer as the ADR question above, same reason: Engineering
Manager does, through the `shell`/`ai-git` access it already holds for orchestration —
a process step, not a new tool grant for Principal-Engineer. Principal-Engineer's
...
those need raw `gh api` scripting, not a simple command) instead of session-only
output — and when it's produced changes too, since the PR now
opens as a draft right after Software-Engineer implements rather than after approval
(check 7).
...
(check 13). Software-Engineer's Tier 3 hand-off re-implements the shape natively.
...
| `AIF-003` epic (decision-record amendment ladder) + its 8 chunk plans                                      | **Abandoned mid-flight.** The ladder it implements is retired by this model. PRs #23/#24 closed unmerged 2026-09-09; chunks `001/003/004` had already merged and need unwinding (checks 1 and 32). Chunk `005` also merged but needs **no** unwinding — check 32 keeps its change (the steering de-enumeration is correct under MADR too).      |
...
| `ARCH-004` Standards sync mechanism                    | Yes, still `Draft`            | Decide or drop before converting (check 26) — its byte-level schema belongs in arc42 §5, not a frozen ADR.                                                                                                                                                                                                                                              |
...
| `ARCH-007` Epic/Chunk → YouTrack                       | Yes, still `Draft`            | Same "decide or drop before converting" gate as `ARCH-004` (check 26) — not called out separately before, but it's in the identical state. Once approved: Core → MADR, `Design` (field schema, workflow rules) → `docs/plans/youtrack-integration-plan.md`.                                                                                             |
...
| `PROC-003` Parallel chunk isolation (worktrees)        | Partly, still `Draft`         | Same gate as `ARCH-004`/`ARCH-007` (check 26). Once approved: short MADR ADR for the worktrees-vs-alternatives trade-off; operational detail → orchestration skill text.                                                                                                                                                                                |
...
| `PLAN-001` Epic/chunk colocation                       | No — convention               | Already deleted from disk; retire the stale index reference (check 30).                                                                                                                                                                                                                                                                                 |
...
and **CI** — `.github/workflows/ci.yml` (lint, typecheck, format, validate, snapshot
freshness, version-bump check, test on Node 22.x/26.x) already exists, landed via
sibling PRs before this document merged; not an open question this work resolves.
Check 33 adds new guards to that workflow.
...
`8b`/`8c`/`13b`/`16b`/`21b` are now plain numbers in sequence). Checks 35 and 36 are
the two exceptions to "numbered in run order" — see their own rows and the
Sequencing section's closing note. More checks (37, 38, ...) may join check 36 in
its own phase if further structural realizations surface while implementing Phase 8
— each one fully specified when it's added, the same way check 36 is, not an
open-ended placeholder. See Sequencing below for how these group into phases.
...
| 2                     | `AIF-004` marked `Deferred` and moved into `archive/` (per Retiring a plan above). Much lighter than `AIF-003`'s teardown (checks 1 and 32): `AIF-004` was never dispatched — no `chunks.json`, no orchestration state, no branches or PRs to unwind. Just the epic file's `Status` flip and a move to `archive/`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| 5                     | `aif index` extended for arc42 sections (entry shape above), for two things §5 and the numbered directory can't give you: the computed `source path → [docs]` reverse index from `key_files`, and the `stale` flag from `last_verified`. Not a forward nav index — arc42's own numbering + §5's block list already cover browsing, so none is built. Decisions stay in scope via the retargeted indexer (check 30) — one module, two doc sets.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 6                     | Agent YAMLs updated per Agent roster: 4 retired, 4 modified, 1 new. `architect.yaml` **loses** `shell` and `web_search`/`web_fetch`, gains only plain `write` (scoped to `docs/decisions/**` + `docs/architecture/**`) plus gated Researcher dispatch — no `@adr/*` tools exist, per ADR tooling above. `engineering-researcher.yaml` scoped for ADR-grade depth, write scoped to `{paths.research}` from check 3. Also states explicitly, in whichever agent dispatches Architect, that it — not Architect — commits the resulting ADR/arc42 file. Everything else in this list derives from this check. **Deliberately still Epic/Chunk-worded:** `engineering-manager.yaml`, `software-engineer.yaml`, and `principal-engineer.yaml` reference `skill/epic-planning`/`skill/chunk-planning`/`skill/chunk-orchestration`/`chunks.json` and say "Epic Plan"/"Chunk Plan" throughout, since checks 9–11 (not this check) own that rename — see their notes for exactly what to sweep.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 7                     | Software-Engineer opens the Task's PR as a **draft** immediately after implementing (right after Process step 11's commit+push), not after Principal-Engineer approval as today — every Task gets a PR whether or not it needs a revision round, giving the review loop a visible record instead of one that only exists in session output. `software-engineer.yaml`'s separate "Process (PR creation — when instructed by Engineering Manager after review approval)" section is removed; opening the PR folds into the implementation process itself. Principal-Engineer's Review Report (still produced per `skill/code-review`/`skill/ai-component-review`/`skill/review-severity`, unchanged) is posted onto that PR as a single PR review (one comment body, not per-finding inline threads) by **Engineering Manager**, not Principal-Engineer itself — Principal-Engineer gains no `shell`/`gh`/`ai-git` access; Engineering Manager posts through the `shell`/`ai-git` access it already holds for orchestration, the same pattern as it already committing an Architect-authored ADR (see Resolved design questions). Correction loops happen on the same PR: Software-Engineer pushes fixes to the same branch, Engineering Manager posts Principal-Engineer's next Review Report as a new review on the updated diff, same 5-iteration cap as today. Once Principal-Engineer approves, Engineering Manager marks the PR ready-for-review (undrafts it) — no second PR is created. Updates the End-to-end flow diagram and the Agent roster table's Principal Engineer row (both above) and `engineering-manager.yaml`'s orchestration Process/Hard rules for the draft-open/post-review/undraft sequence. **Deliberately still Chunk/Epic-worded**, same reason as check 6 — checks 9–11 own that rename, not this check. `skills/chunk-orchestration/SKILL.md` is deliberately left untouched too, same reason as check 6 left it alone — it still instructs creating the PR only after Principal-Engineer approves, now stale; check 10's full pipeline rewrite is where that gets fixed (see its note). |
| 8                     | Drop Software-Engineer's blanket CHANGELOG requirement (process step 9, "A CHANGELOG entry is mandatory for every completed Chunk Plan" hard rule, and the `## [{date}] {Plan ID} — {Title}` format) — no `CHANGELOG.md` exists anywhere in this repo despite the rule, dead process since before this rework (see the Decisions section's new callout above for the full reasoning). Reword the remaining "Engineering documentation only in READMEs/CHANGELOGs — no product copy" hard rule to drop the now-removed CHANGELOG reference; the README half and the no-product-copy rule both stay. Add an optional "Release Documentation" section to `projects/_template/project-standards.md` (Keeps a CHANGELOG? / Location / Format / Entry trigger, unfilled by default) so a project that ships versioned releases can opt in — Software-Engineer needs no new conditional logic to honor it, since "follow the active standards file" is already a hard rule. No version bump needed for `project-standards.md` (no frontmatter `version` field on that file).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 9                     | `epic-planning` + `chunk-planning` merge into `feature-planning`: renamed, Task-sizing rules added, small Features may skip decomposition. **Also sweep check 6's agent YAMLs — do not skip them because they look freshly written.** `engineering-manager.yaml`, `software-engineer.yaml`, and `principal-engineer.yaml` deliberately still say `skill/epic-planning`/`skill/chunk-planning` and "Epic Plan"/"Chunk Plan" throughout their prompts and hard rules — not only in their `skills:` lists — because check 6 landed before this rename and was instructed to keep the pre-rename names rather than jump ahead of checks 9–11. Update every occurrence in those three files, not a representative few. **`skills/epic-planning/SKILL.md` and `skills/chunk-planning/SKILL.md` themselves are untouched by check 6** and still fully describe the pre-roster world: literal "Tech-Lead"/"AI-Engineer"/"Test-Engineer" agent names, and a software-track-vs-AI-track split gated by `chunks.json`'s `agents` field. Check 6 deliberately left them alone rather than partially patch agent names into a track structure check 10 was already going to remove — do not mistake that silence for "already fine." This merge is where their content, not just their names, gets rewritten.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 10                    | `chunk-orchestration`: `chunks.json` → `tasks.json`; per-Task plan gate replaced by `complexity-tiers`; software/AI-track branching removed — one pipeline (implement+self-test+docs → PE review) for every Task. **`skills/chunk-orchestration/SKILL.md` itself is untouched by check 6** and still fully implements the two-track pipeline this check removes (`AI-Engineer`/`Test-Engineer` named throughout, track branching keyed on `chunks.json`'s `agents` field) — same reason as check 9's note: check 6 deliberately did not partially rename agents into a structure this check was already going to delete. **Also sweep, same reason as check 9's note:** `engineering-manager.yaml`'s `skill/chunk-orchestration` reference, every `chunks.json` mention in its Process/Hard rules sections, and its `EVERY CHUNK GETS ITS OWN BRANCH AND WORKTREE` hard rule (and the branch-naming pattern `{epic-id}/{chunk-id}-{short-description}` beside it) — all still Chunk-worded by the same deliberate check-6 deferral. **One more thing this rewrite must fix, not just rename:** the skill's per-chunk pipeline still instructs dispatching an agent to run `gh pr create` only after Principal-Engineer approves ("Record the PR number and URL," "worktree remains active until the human confirms the PR is merged") — that's the pre-check-7 timing. Check 7 moved PR creation to Software-Engineer's first implementation pass (as a draft) and review delivery to Engineering-Manager posting onto that PR, but left `skill/chunk-orchestration/SKILL.md` itself untouched for the same reason as check 6 did — this check's full pipeline rewrite is where that mechanic gets corrected too, not a separate pass.                                                                                                                                                                                                                                                                                                                                                                                 |
| 11                    | DAG server + `lib` renamed chunk→task; wave output identical for an equivalent graph. Includes `servers/dag/logic.js`'s doc comment and the chunk/epic-worded test fixtures in all three `servers/dag/tests/**` files, not just identifiers.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 12                    | What `knowledge-authoring` carried is re-homed: a steering rule holds the four-kind routing table; templates hold the file shapes (MADR, arc42 section, external-reference file). A skill earns its place with judgment/validation steps; a template + always-on rule is enough for light-structure prose. Resolved before check 13 so that check knows whether `docs/knowledge-file-format.md` still needs a body.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 13                    | `decision-triage`, `decision-brief`, `decision-record`, `chunk-planning`, `ai-engineering-plan`, `knowledge-authoring` skills deleted, all references removed. `decision-record` because MADR replaces (not shrinks) it; `knowledge-authoring` per Subject matter is not a document kind (check 12). `docs/knowledge-file-format.md` — the live canonical spec for the exact `type: decision\|reference\|architecture\|api\|business-rule` taxonomy this section retires — rewritten to match (or deleted if check 12's steering rule + templates fully replace what it specified); it's actively pointed to from `projects/_template/knowledge/example.md` and several `docs/plans/*.md` files, which need the same update. **Also retired here, not left behind as dead wiring once the taxonomy above is gone:** `lib/knowledge.js` (`validateKnowledgeFrontmatter`/`buildIndexEntry`), `KNOWLEDGE_TYPES` in `lib/constants.js`, and the `knowledge` target of `aif index` in `lib/commands/index.js` — by check 5's landing, decisions and architecture each already have their own dedicated indexer, and this repo's own corpus has exactly one file still using the generic one (`docs/agent-prompt-extraction-candidates.md`, `type: reference`), which check 18 already sweeps and doesn't fit the taxonomy's actual surviving scope (external reference material about _other_ systems, routed to `steering/` per check 12 — not this repo's own prompt-authoring debt tracking). Migrate that one file's frontmatter (or fold its content elsewhere) as part of this check so nothing depends on the code being deleted.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 14                    | `plan-lifecycle`/`complexity-tiers` trimmed; `complexity-tiers` re-pointed as SE's primary gate. Tier 3 becomes "stop and hand off" (orchestrated → EM, standalone → human). `plan this` documented as a Tier 2 floor, not a Tier 3 jump.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 15                    | `projects/_template/` brought to the new format — every consuming project starts from this skeleton. Not just the old `epics/chunks/orchestration` layout renamed: the **full** new document layout, mirroring what checks 3–14 just built. `.aiconfig.json`'s new `paths.*` fields (check 3) filled in for the template itself. `plans/{features,tasks,orchestration}/` (renamed from `epics/chunks/orchestration`, per checks 9–10). `knowledge/decisions/` (flat, MADR-ready — mirrors `docs/decisions/`) and `knowledge/architecture/` (empty; arc42 sections created lazily per check 4, never pre-scaffolded) and `knowledge/research/` (the `paths.research` target). **No** `knowledge/product/` directory — the Product doc type stays a reserved slot with no owner and no folder until a product agent lands; scaffolding an empty one is exactly the premature ceremony this model exists to avoid. `project-standards.md`'s references to the skills retired in check 13 updated.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 16                    | `steering/engineering/core.md` Rules 1/2/8/9 reworded (Rule 1 → `complexity-tiers` gate, Rule 2 gets a Tier 1/2 fallback, Chunk/Epic Plan → Feature Plan); `knowledge-consumption.md` moves decision-records from full-body auto-load to **index-only by default, full body on demand**; doc-update acceptance gate added — explicit that this includes **adding** a `key_files` entry for a new file extending a described building block, not just editing prose for files already listed, since a file's absence from every `key_files` list is not evidence it's out of scope, only that nobody has checked yet (closes the gap check 5/33's staleness detection can't: it only watches files already listed).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 17                    | **Full vocabulary + reference sweep**, not just planning skills: `skills/code-review/`, `skills/test-execution/`, `skills/worktree-management/SKILL.md`, `skills/complexity-tiers/SKILL.md`, `skills/plan-lifecycle/reference/commit-gate-procedure.md`, `standards/csharp_base.md`, `standards/javascript_base.md`, `steering/engineering/git-workflow-projects.md`. Deleted-skill references also in `skills/agent-authoring/reference/schema.md`, `projects/_template/project-standards.md`, `standards/javascript_node.md`, and `lib/resolver.js`'s JSDoc examples.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| 18                    | `skill/agent-authoring` and `docs/agent-prompt-extraction-candidates.md` swept. Product-doc ownership **not** assigned — reserved and unbuilt.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 19                    | `skills/agent-authoring/reference/tools.yaml` gains the trifecta-avoidance rule (no agent holds `moderate`/web and `privileged`/write+shell at once without documented isolation).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 20                    | `skill/code-review`'s checklist gains two explicit items, since the agent schema has no path-scoping field to enforce either instead (see Agent roster's "reviewed convention, not schema-enforced" note): (1) for any diff where Architect or Engineering Researcher performed the `write`, Principal-Engineer confirms the touched paths stay within their documented scope — `docs/decisions/**`/`docs/architecture/**` for Architect, `{paths.research}` for Researcher — a violation is a HIGH finding, same severity class as the existing `tools`/`approved_tools`/`blocked_commands` rule (Why Principal-Engineer absorbs AI-component review, above); (2) for any diff adding a file under a directory an arc42 section already describes, confirm that section's building-block table and `key_files` were updated to include it — a MEDIUM finding otherwise, the same silent-drift class check 5/33's staleness detection exists to catch, just upstream of where that mechanism can ever see the file (check 16).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 21                    | Software-Engineer's hard rules state a Researcher brief is data informing a decision, never an instruction to execute; confirm PE review applies regardless of who dispatched SE.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 22                    | `bundles/engineering/snapshot.json` regenerated (`bundle.yaml` needs no edit — domain-based auto-discovery absorbs the shrink). Run only after checks 6 and 13 have actually landed, or the snapshot still reflects the pre-shrink roster.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 23                    | `README.md`, `PLAN.md`, `AGENTS.md`, `agents/README.md`, `skills/README.md` updated. `install.ps1` not in scope — already deleted (dead per `ARCH-001`), its `tools.test.js` assertion removed with it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| 24                    | `tests/validation/` cross-reference check passes against the new set; `npm test` green. Fixtures cleaned up: `tests/unit/decisions.test.js`, `tests/integration/decisions-index.test.js`, `knowledge-index.test.js`, `base.test.js`, `claude-adapter.test.js`, `kiro-adapter.test.js`, `tests/unit/resolver.test.js`, `tests/integration/resolver.test.js`, `tests/integration/commands.test.js`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 25                    | Terminology-sweep verification as an automated `tests/validation/` check: repo-wide `chunk`/`epic` search returns zero matches outside `docs/plans/{chunks,epics,orchestration}/`, `docs/plans/completed/`, `docs/plans/archive/`, MADR record bodies (e.g. `ARCH-007`), and `docs/decisions/index.json` (generated from those bodies, not itself a body). `agent-consolidation-plan.md` and `AIF-004.epic.md` are covered as kept-in-place history. The two already-deleted plans need no exception. Runs last, once checks 3–23 have landed — including check 15, so `projects/_template/` is clean too.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 26                    | Human decision gate for every source record still `Status: Draft` at conversion time — currently `ARCH-004`, `ARCH-007`, `PROC-003` (verified against `docs/decisions/index.json` and each file's own Metadata table). Each gets an explicit `Approved`/`Deferred` call before its MADR conversion (check 29 or 31) proceeds — `Draft` never silently becomes an accepted MADR record. A `Deferred` verdict drops that record from this pass entirely rather than converting it speculatively.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 27                    | Existing decision records dispositioned per the table above; archive location created.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 28                    | `docs/decisions/` flattened (domain subfolders removed, flat bare-number MADR counter — `0007-slug.md`, no domain-coded `AIF-ARCH-`/`AIF-PROC-`/etc. prefix); every surviving record rewritten by hand into MADR within the word budget, renumbered onto the new counter, checked at PE review.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 29                    | `Design` sections split out of `ARCH-001/002/003/007` into their arc42 homes (or the YouTrack plan for `007`), plus `ARCH-004` if check 26 approves it; `ARCH-005/006` converted whole per their disposition-table exceptions. This is what seeds `docs/architecture/` with real content.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 30                    | `lib/decisions.js` **retargeted, not deleted**: parser moves from the `## Metadata` table to MADR frontmatter, keeping index generation and the `supersedes`→`superseded_by` inversion. `aif index decisions` and `docs/decisions/index.json` stay. `tests/unit/decisions.test.js` and `tests/integration/decisions-index.test.js` updated to new fixtures.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 31                    | The three New ADRs to write are written in MADR form.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 32                    | Merged half of `AIF-003` unwound, minus what already dies elsewhere. Real revert: `AIF-003-004`'s `Amending` status, removed from `plan-lifecycle/reference/status-vocabulary.md` by hand. `AIF-003-003`'s Amendments/Errata template sections die with check 13; `AIF-003-001`'s `Supersedes` parse dies with check 30 — no separate revert needed. `AIF-003-005`'s steering de-enumeration is **kept** (correct under MADR too).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 33                    | New guards added to the **existing** `.github/workflows/ci.yml` (already runs lint/typecheck/format/validate/snapshot-freshness/version-bump-check/test): the `key_files`-vs-`last_verified` staleness check, relative-link resolution across `docs/architecture`, `aif index decisions --check`. No `adrs-core lint` or hyphenated-`kind` guard — both moot while ADRs are hand-written. Any steering/agent file this rework touches (e.g. `steering/engineering/core.md`, `steering/global/knowledge-consumption.md`, `agents/*.yaml` per check 6) needs its `version` field bumped alongside the content change, or the existing version-bump-check fails it; files deleted outright (check 13) are unaffected.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 34                    | Freeform `docs/plans/*.md` triaged and cleared; `docs/plans/completed/` holds the finished ones.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 35 _(deferred)_       | A **deferred** ADR, written by Architect once there's enough hand-written volume: ADR tooling (Rust CLI vs. in-repo JS vs. staying manual — see ADR tooling above, and `docs/plans/adr-kit-plan.md`). Gates nothing — deliberately outside the sequence below.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 36 _(see Sequencing)_ | Collapse the three parallel top-level directories (`plans/features/`, `plans/tasks/{FeatureID}/`, `plans/orchestration/{FeatureID}/`) into one folder per Feature: `plans/features/{FeatureID}/{plan.md,tasks.json,orchestration-state.json}`. Tasks no longer carry individual plan files (check 9's resolved design question — a per-Task plan, when one exists at all, is the implementing agent's own tier-driven output, never a `feature-planning` deliverable), so `tasks.json` and `orchestration-state.json` are just two lightweight sibling artifacts to the Feature Plan itself; three top-level directories keyed off the same FeatureID is a holdover from the old Epic/Chunk/Orchestration split that no longer earns its keep. Revises, not just extends, already-landed work: `.aiconfig.json` schema (check 3 — collapse `paths.tasks`/`paths.orchestration` into `paths.features`; decide at execution time whether the old keys are dropped outright or kept as deprecated aliases for one release), `skills/feature-planning/` and `skills/task-orchestration/`'s Location/Outputs sections and asset templates (checks 9/10), `AGENTS.md`'s field table, this repo's own `.aiconfig.json`. Also touches `docs/architecture/02_constraints.md`'s Artifact file naming convention row (a `key_files`-tracked doc — needs its own `last_verified` bump once edited) — moving the Feature Plan from `{FeatureID}.feature.md` to a bare `plan.md` inside a `{FeatureID}/` folder changes what "suffix alone identifies the file" means: type still lives in the filename, identity moves to the parent directory name instead. Confirm that reading is an acceptable extension of the existing convention, or reword it — don't drift past it silently.                                                                                                                                                                                                                                                                                                                                               |
...
5. **PR/review flow** (check 7) — Software-Engineer opens the Task's PR as a draft
   right after implementing instead of after approval; Principal-Engineer's Review
   Report is posted onto it by Engineering Manager. Depends on phase 4 — modifies the
   same agent YAMLs check 6 just landed.
6. **CHANGELOG mechanism cleanup** (check 8) — drops Software-Engineer's dead-process
   CHANGELOG requirement, adds an opt-in Release Documentation section to the project
   template. Depends on phase 4 — modifies the same agent YAMLs, same reason as phase 5.
7. **Vocabulary rename** (checks 9–11) — Feature/Task terminology, then the DAG/lib
   chunk→task rename.
8. **Skill retirement & re-homing** (checks 12–14) — decide where `knowledge-authoring`'s
   content goes first, then delete the skills that decision retires, then trim
   `plan-lifecycle`/`complexity-tiers`. A **structural-review phase** (check 36,
   possibly joined by 37/38/... if more turn up) follows this phase, before phase 9
   begins — see its own row and the closing note below.
9. **Template alignment** (check 15) — `projects/_template/` rebuilt on the full new
   layout. Depends on the vocabulary rename (phase 7), skill retirement (phase 8),
   and the structural-review phase (check 36 and any siblings added to it, landed
   between phases 8 and 9) it's built to reflect.
10. **Steering & reference sweep** (checks 16–18) — core steering rules first, then the
    wider vocabulary/reference sweep, then `agent-authoring`.
11. **Cross-cutting process/security rules** (checks 19–21) — the trifecta-avoidance
    rule, the code-review path-scoping check, and Software-Engineer's Researcher-brief
    rule.
12. **Regeneration & top-level docs** (checks 22–23) — the bundle snapshot regenerates
    only once the roster and skill shrink (phases 4 and 8) are actually on disk;
    top-level docs update last so they describe the settled state.
13. **Verification** (checks 24–25) — `npm test` green first, then the terminology
    sweep, once every check in phases 2–12 has landed.
14. **Decisions conversion** (checks 26–31) — resolve the Draft-record gate first,
...
15. **Unwind the merged half of `AIF-003`** (check 32) — small by this point; depends
    on phase 8 (skill retirement) and phase 14 (decisions retargeting).
16. **CI guards** (check 33) — added once there's a converted decision log and
    populated arc42 sections to actually run against.
17. **Freeform plan triage** (check 34) — last, once the rest of the repo has settled.

Check 35 (ADR tooling) is deliberately outside this sequence — Architect's to write
later, gates nothing. Check 36 opens a **structural-review phase** also outside the
original 17-phase numbering — added after the fact once implementation surfaced a
structural realization checks 1–35 didn't anticipate — but unlike check 35, this
phase does gate something: it runs between phases 8 and 9, and phase 9 must not
begin until every check in it has landed. If further structural realizations
surface while implementing phase 8, they land as additional fully-specified checks
in this same phase (37, 38, ...) — each one a normal check like any other in this
table, not an addition to check 36 itself. Only the phase stays open until phase 9
begins; every check inside it is as concrete and fixed as check 36.
