 ...
 *
 * Plan: AIF-002-014 (added the -k/-d flag split, resolveDecisionsPath, and
 * the -d/--decision decision-index generation path; the pre-existing
 * -k/knowledge-index logic below is unmodified in behavior).
 *
 * Usage:
 *   aif index -k/--knowledge          — generate knowledge/index.json
 *   aif index -d/--decision           — generate {paths.decisions}/index.json
 *   aif index -d/--decision --check   — verify the decision index without writing
 */
 ...
 * @returns {{ generated_at: string, entries: object[] }|null}
 */
...
export function runIndex(parsed, repoRoot) {
  const hasKnowledge = Boolean(parsed.args.k || parsed.args.knowledge);
  const hasDecision = Boolean(parsed.args.d || parsed.args.decision);

  if (hasKnowledge && hasDecision) {
    console.error('Error: aif index accepts only one of -k/--knowledge or -d/--decision, not both.');
    return 1;
  }
  ...
  console.error('Error: aif index requires -k/--knowledge or -d/--decision.');
  return 1;
}
